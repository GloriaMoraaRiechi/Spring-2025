...
// Function to simulate GFR drop with risk factor adjustments
double simulate_GFR_drop(double currentGFR, int age, bool has_diabetes, bool has_hypertension) {
    double drop = (rand() % 4 + 2); // Default drop between 2 and 5 mL/year

    // Increase the drop if patient has risk factors
    if (has_diabetes || has_hypertension) {
        drop += (rand() % 3 + 1); // Extra drop due to diabetes or hypertension
    }

    // Additional drop for older patients
    if (age > 60) {
        drop += (rand() % 3 + 1); // Older patients experience more rapid progression
    }
...
// Function to log events with timestamps into a file
void log_event(ofstream& log_file, const string& event) {
    time_t now = time(0);
    char* dt = ctime(&now);
    log_file << "[" << dt << "] " << event << endl;
}

// Function to calculate survival curve statistics
void calculate_statistics(ofstream& stats_file, const vector<int>& progression_times) {
    double sum = 0;
    for (int time : progression_times) {
        sum += time;
    }
    double avg_time = sum / progression_times.size();
    stats_file << "Average progression time to ESRD or death: " << avg_time << " years" << endl;
...
int main() {
    ...
    // Open log file for saving the events
    ofstream log_file("ckd_simulation_log.txt");
    if (!log_file.is_open()) {
        cerr << "Could not open log file!" << endl;
        return -1;
    }

    // Open statistics file for saving the progression stats
    ofstream stats_file("ckd_statistics.txt");
    if (!stats_file.is_open()) {
        cerr << "Could not open statistics file!" << endl;
        return -1;
    }
    ...
    for (auto& patient : allPatients) {
        log_event(log_file, "Patient " + to_string(patient.PatientID) + " started with stage " + to_string(patient.Stage));
        
        int time_to_esrd_or_death = 0;
        while (patient.GFR > 15 && patient.Stage < 5) { // Simulate until ESRD or death (Stage 5)
            patient.GFR = simulate_GFR_drop(patient.GFR, patient.Age, patient.FamilyHistoryDiabetes, patient.FamilyHistoryHypertension);
            time_to_esrd_or_death++;

            // Check if hospitalization occurs
            if (check_for_hospitalization(patient.GFR)) {
                log_event(log_file, "Patient " + to_string(patient.PatientID) + " hospitalized at GFR: " + to_string(patient.GFR));
            }
            ...
            log_event(log_file, "Patient " + to_string(patient.PatientID) + " transitioned to Stage " + to_string(patient.Stage));
        }

        // Simulate ESRD after Stage 5 and trigger event after X weeks
        if (patient.Stage == 5) {
            log_event(log_file, "Patient " + to_string(patient.PatientID) + " reached Stage 5 (ESRD) and will progress to ESRD after X weeks");
        }
    ...
    calculate_statistics(stats_file, progression_times);

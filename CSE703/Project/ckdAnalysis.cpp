...
// Comparator for priority_queue to create a min-hip based on GFR
struct compareGFR
{
	bool operator()(const CKD_Patient& p1, const CKD_Patient& p2)
	{
		return p1.GFR > p2.GFR;  // Lowest GFR at the root
	}
};

// Function to detremine the CKD stage based on the GFR value
int getCKDStage(double GFR)
{
	if (GFR >= 90) return 1;
	else if (GFR >= 60) return 2;
	else if (GFR >= 45) return 3;
	else if (GFR >= 30) return 4;
	else if (GFR >= 15) return 5;
	else return 6;
}
...
int main()
{
        ...
        getline(ss, value, ','); patient.Ethnicity = stoi(value);
		...
		// Assing CKD stage based on GFR
		patient.Stage = getCKDStage(patient.GFR);

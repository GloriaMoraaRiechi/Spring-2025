...
void simulate_event(CKD_Patient& patient, double time_step) {
    ...
    if (time_step >= 1.0) {  // Assuming yearly progression
        ...
        // Ensure GFR does not go below 0 and adjust Stage accordingly
        if (patient.GFR <= 0) {
            patient.GFR = 0;  // Cap GFR at 0
...
void process_events(vector<CKD_Patient>& patients) {
    ...
    while (!eventQueue.empty()) {
        ...
        schedule_checkup(patient, currentTime);
		        ...
		        // Stop if all patients are in Stage 5
        bool allInStage5 = true;
        ...
        if (allInStage5) {
            cout << "All patients have reached Stage 5. Stopping simulation." << endl;
            break;
        }
